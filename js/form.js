...
var Select = {
    ...
    select: function(el) {
        ...
        if (_f.hasClass('form__select_multiple')) {
            ...
            toInputValue = _._el.attr('data-value')?  _._el.attr('data-value'): _._el.text();
        ...
        if (_._el.closest('.form_map').length) {
            $('.form_map').submit();
        }

        if (_._el.closest('.calc__select').length) {
            Calc.choice();
        }
...
$('body').on('click', '.form__text-input_autocomplete, .form__textarea_autocomplete', function() {
    $('.form__select').removeClass('form__select_opened');
    $('.form__select-options').slideUp(221);
    Select.autocomplete(this);
});
...
function setTextareaHeight(_$, focus) {
    var val = _$.val(),
    Shape = _$.parent().find('.form__textarea-shape');

    if (focus) {
        _$.val('').parent().next('.form__select-options').find('li').removeClass('hidden');
        Shape.html('');
    }

    Shape.html(val).css('width', _$.innerWidth());
...
$('body').on('focus', '.form__textarea_var-h', function() {
    setTextareaHeight($(this), true);
});
...
//validateForm
    var Form = {
        ...
        email: function () {
            var _ = this,
                err = false;
            if (!/^[a-z0-9]+[-_a-z0-9\.]*@[-_a-z0-9]{2,}\.[a-z]{2,6}$/i.test(_.input.val())) {
                _.error(true, true);
                err = true;
            } else {
                _.error(false);
    ...
    var processJson = function (response) {
        ...
        var delay = data.delay ? data.delay : 2000;
        if (data.text) {
            Popup.message('#message-popup', data.text);
        }
        if (data.redirect) {
            setTimeout(function () {
    ...
    Form.submit('.auth_form', function (form, callback) {
        var _f = $(form);
        $.ajax({
            url: _f.attr('action'),
            type: "POST",
            dataType: false,
            procesDataType: false,
            data: _f.serialize(), //new FormData(form),
            success: function (response) {
                if(_f.hasClass('recover__form')) { // for recover password
                    if($(response).find('font.errortext').length > 0) {
                        _f.find('.form__error-tip').show().html($(response).find('font.errortext').html().replace("Логин или EMail не найдены.", "Пользователь с данным адресом не зарегистрирован."));
                        setTimeout(function () {
                            _f.find('.button').removeClass('form__button_loading');
                        }, 300)
                    } else {
                        Popup.message('#message-popup', $(response).find('font.notetext').html());
                        _f.find('.form__error-tip').hide();
                    }
                    callback(true);
                } else {
...
$('body').on('click', '.auth_btn', function (e) {
    e.preventDefault();
    var _ = $(this),
    cl = _.attr('href').slice(1);
    $('#login-block .auth_form,.auth_btns').hide();
    $('#login-block .'+cl+'__form').show();
    $('#login-block .'+cl+'__btn').show();

});

    Form.submit('.form_rev', function (form, callback) {
        var _f = $(form);
        $.ajax({
            url: _f.attr('action'),
            type: "POST",
            ...
            data: _f.serialize(),
            success: function (response) {
                $('.content__num_sm.chel').html($(response).find('.content__num_sm.chel').html());
                Popup.message('#message-popup', $(response).find('font.notetext').html());
                setTimeout(function () {
                    _f.find('.button').removeClass('form__button_loading');
                }, 300);
    ...
    Form.submit('.kpr-ajax-form', function (_el, callback) {
        var _f = $(_el);
        //Popup.message('#message-popup', 'Форма отправлена');
        //$.post(_f.attr('action'), new FormData(_el))
        ...
        $.ajax({
            ...
            procesData: false,
            procesDataType: false,
            data: _f.serialize(), //new FormData(_el),
            success: function(response){
                $(_el).find('.form__textarea, .form__text-input').val('');
                $(_el).find('.overlabel-apply').attr('style','');
                Popup.message('#message-popup', 'Форма отправлена');
                callback(true);
            },
            error: function() {
    ...
    Form.submit('.subscribe-form', function (form, callback) {
        var _f = $(form);
        //Popup.message('#message-popup', 'Форма отправлена');
        $.ajax({
         url: _f.attr('action'),
         ...
         data: _f.serialize(), //new FormData(form),
         success: function(response){
         Popup.message('#message-popup', 'Подписка оформлена');
         callback(true);
         },
         error: function() {
         alert('Send Error');
    ...
    Form.submit('.open-count-form', function (form, callback) {
        var _f = $(form);
        $.ajax({
            url: _f.attr('action'),
            type: "POST",
    ...
    Form.submit('.form_quiz', function (form, callback) {
        var _f = $(form);
        _f.addClass('form_closed').find('input').prop('disabled', true);
        Popup.message('#message-popup', 'Ответ отправлен');
        callback(true);
    });


    $('body').on('keyup', '.invoice-form .form__text-input', function() {
        console.log('ku');
        var _$ = $(this);
        if (/^[0-9 ]*?[\.,-]?[0-9]{0,2}$/.test(_$.val())) {
    ...
    Form.submit('.invoice-form', function (form, callback) {
        var _f = $(form);
        var myWindow=window.open('/personal/order.php?'+_f.serialize(),'','width=753,height=600');
        myWindow.focus();
        myWindow.print();
        callback(true);
    });

    Form.submit('.question-form', function (form, callback) {
        var _f = $(form);
        $.ajax({
            url: _f.attr('action'),
            type: "POST",
    ...
    Form.submit('.form_change-email', function (form, callback) {
        var _f = $(form);
        $.ajax({
            url: _f.attr('action'),
            type: "POST",
    ...
    Form.submit('.js-form-upd-personal-info', function (form, callback) {
        var _f = $(form);
        $.ajax({
            url: _f.attr('action'),
            type: "POST",
    ...
    $(document).on('change', '#search-i-txt-1', function(){
        $('label[for="search-i-txt-1"]').css('opacity', 0);
    });
